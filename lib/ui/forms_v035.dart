            ...
            children: [
              ...
              if (showAdvanced) ...[
                ...
                if (editing == null && type == TransactionType.expense) ...[
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text(
                      'Distribui automaticamente as próximas parcelas',
                ...
                if (editing == null && !installment) ...[
                  SwitchListTile(
                    ...
                    subtitle: const Text(
                      'Cria as próximas ocorrências no planejamento',
                      style: TextStyle(fontSize: 8.5),
                    ),
                    value: recurring,
                  ...
                  if (recurring) ...[
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (value) {
                        if (value != null)
                          setLocal(() => recurrenceDuration = value);
                      },
                    ),
                    ...
                    if (recurrenceDuration == 'data') ...[
                      ...
                      InkWell(
                        onTap: () async {
                          final picked = await pickFinoraDate(
                            sheetContext,
                            recurrenceEndDate,
                          );
                          if (picked != null)
                            setLocal(() => recurrenceEndDate = picked);
                        },
                        child: InputDecorator(
                          decoration: const InputDecoration(
              ...
              SizedBox(
                ...
                child: FilledButton(
                  onPressed: () {
                    ...
                    if (editing != null) {
                      ...
                      final maxOccurrences = recurrenceDuration == 'quantidade'
                          ? (int.tryParse(recurrenceCountController.text) ?? 12)
                                .clamp(2, 120)
                          : null;
                      savedOk = store.addRecurring(
                        ...
                        frequency: frequency,
                        startDate: date,
                        endDate: recurrenceDuration == 'data'
                            ? recurrenceEndDate
                            : null,
                        maxOccurrences: maxOccurrences,
                      );
                    } else {
                      savedOk = store.addTransaction(
                        TransactionItem(
                    ...
                    if (!savedOk) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Não foi possível salvar. Confira valor, origem e dados do lançamento.',
                          ),
                        ),
                      );
                      return;
                  ...
                  child: Text(
                    editing == null ? 'Salvar lançamento' : 'Atualizar',
                  ),
                ),
              ),
            ],
                ...
                child: FilledButton(
                  onPressed: () {
                    ...
                    if (isCard) {
                      ...
                      if (card != null)
                        invoice = store.invoiceMonthForPurchase(card, date);
                    }
          ...
          children: [
            ...
            if (item.paymentKind == PaymentKind.card)
              detailRow(
                context,
                'Fatura',
                monthLabel(store.transactionInvoiceMonth(item)),
              ),
            ...
            if (item.recurrenceId != null)
              detailRow(context, 'Tipo', 'Recorrente'),
            if (item.note.isNotEmpty)
              detailRow(context, 'Observação', item.note),
            const SizedBox(height: 12),
            Row(
              children: [
          ...
          children: [
            ...
            if (store.selectedIsPast && snapshot != null) ...[
              ...
              if (store.currentCashShortfall > 0) ...[
                const SizedBox(height: 10),
                Text(
                  'Déficit previsto: ${money(context, store.currentCashShortfall)}',
                  style: const TextStyle(
                    color: FinoraColors.expense,
                    fontWeight: FontWeight.w800,
                  ),
              ...
              if (store.suggestedGoalContribution > 0) ...[
                const SizedBox(height: 10),
                Text(
                  'Sugestão opcional para metas: ${money(context, store.suggestedGoalContribution)}',
                  style: const TextStyle(
...
Widget breakdownRow(
  ...
  child: Row(
    children: [
      ...
      Text(
        '${positive ? '+' : '−'} ${money(context, value.abs())}',
        style: TextStyle(
          color: color,
          fontSize: 10.5,
          fontWeight: FontWeight.w900,
        ...
        child: Column(
          ...
          children: [
            ...
            if (item.invoiceMonth != null)
              detailRow(context, 'Fatura', monthLabel(item.invoiceMonth!)),
                  ...
                  onPressed: () {
                    ...
                    if (ok) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Não foi possível realizar este previsto. Verifique a conta ou o cartão vinculado.',
                          ),
                        ),
                      );
                    }
          ...
          children: [
            ...
            if (item.maxOccurrences != null)
              detailRow(
                context,
                'Duração',
                '${item.maxOccurrences} ocorrências',
              ),
            if (item.endDate != null)
              detailRow(context, 'Até', fullDate(item.endDate!)),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
              ...
              child: TextButton.icon(
                onPressed: () async {
                  final ok = await confirmAction(
                    context,
                    'Excluir recorrência?',
                    'As previsões futuras serão marcadas como ignoradas.',
                  );
                  if (!ok) return;
...
Future<void> showRecurringEditForm(
  BuildContext context,
  RecurringRule item,
) async {
  ...
  final count = TextEditingController(
    text: (item.maxOccurrences ?? 12).toString(),
  );
  var category = item.category;
  var frequency = item.frequency;
  var duration = item.endDate != null
      ? 'data'
            ...
            children: [
              ...
              if (duration == 'quantidade') ...[
                ...
                TextField(
                  controller: count,
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(labelText: 'Ocorrências'),
                ),
              ],
              if (duration == 'data') ...[
                const SizedBox(height: 9),
                InkWell(
                  onTap: () async {
                    final picked = await pickFinoraDate(sheetContext, endDate);
                ...
                child: FilledButton(
                  onPressed: () {
                    ...
                    store.updateRecurring(
                      ...
                      maxOccurrences: duration == 'quantidade'
                          ? (int.tryParse(count.text) ?? 12).clamp(2, 120)
                          : null,
                    );
                    Navigator.pop(sheetContext);
                  },
              ...
              child: TextButton.icon(
                onPressed: () async {
                  final ok = await confirmAction(
                    ...
                    'As parcelas já realizadas serão mantidas e as demais ficarão como ignoradas.',
                  );
                  if (!ok) return;
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  ...
                  store.transfer(amount: value, from: from, to: to, date: date);
                  Navigator.pop(sheetContext);
                },
...
Future<void> showBudgetForm(BuildContext context, {BudgetItem? editing}) async {
  ...
  final limit = TextEditingController(
    text: editing == null ? '' : editing.limit.toStringAsFixed(2),
  );
  await showModalBottomSheet<void>(
    context: context,
    showDragHandle: true,
    ...
    builder: (sheetContext) => Padding(
      ...
      child: StatefulBuilder(
        builder: (_, setLocal) => Column(
          ...
          children: [
            Text(
              editing == null ? 'Orçamento mensal' : 'Editar orçamento',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900),
            ),
            const SizedBox(height: 13),
            DropdownButtonFormField<String>(
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  final value = parseNumberInput(limit.text) ?? 0;
                  if (value <= 0) return;
                  if (editing == null) {
                    store.addBudget(category, value);
                  } else {
                    store.updateBudget(editing, category, value);
                  }
                  Navigator.pop(sheetContext);
                },
...
Future<void> showGoalForm(BuildContext context, {GoalItem? editing}) async {
  ...
  var deadline =
      editing?.deadline ?? DateTime.now().add(const Duration(days: 365));
  await showModalBottomSheet<void>(
    context: context,
    showDragHandle: true,
    isScrollControlled: true,
    builder: (sheetContext) => Padding(
      ...
      child: StatefulBuilder(
        builder: (_, setLocal) => Column(
          ...
          children: [
            Text(
              editing == null ? 'Nova meta' : 'Editar meta',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900),
            ),
            const SizedBox(height: 13),
            TextField(
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Valor guardado'),
            ),
            const SizedBox(height: 9),
            InkWell(
              onTap: () async {
                final picked = await pickFinoraDate(sheetContext, deadline);
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  final tv = parseNumberInput(target.text) ?? 0;
                  final sv = parseNumberInput(saved.text) ?? 0;
                  if (name.text.trim().isEmpty || tv <= 0) return;
                  if (editing == null) {
                    store.addGoal(name.text.trim(), tv, sv, deadline);
                  } else {
                    store.updateGoal(
                      editing,
                      name.text.trim(),
...
Future<void> showReserveForm(
  ...
  await showSimpleForm(
    context,
    editing == null ? 'Nova reserva' : 'Editar reserva',
    [
      FormFieldData('Nome', name),
      FormFieldData('Valor alvo', target, number: true),
      FormFieldData('Valor guardado', saved, number: true),
      FormFieldData('Meses de proteção', months, number: true),
    ],
    () {
      final tv = parseNumberInput(target.text) ?? 0;
      final sv = parseNumberInput(saved.text) ?? 0;
      final mv = int.tryParse(months.text) ?? 6;
      if (name.text.trim().isEmpty || tv <= 0) return false;
      if (editing == null) {
        store.addReserve(name.text.trim(), tv, sv, months: mv);
      } else {
        store.updateReserve(editing, name.text.trim(), tv, sv, mv);
      }
      return true;
    },
  );
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: classes
                  ...
                  .toList(),
              onChanged: (v) {
                if (v != null) setLocal(() => assetClass = v);
              },
            ),
            const SizedBox(height: 9),
            TextField(
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  final av = parseNumberInput(amount.text) ?? 0;
                  final rv = parseNumberInput(estimated.text) ?? 0;
                  if (name.text.trim().isEmpty || av <= 0) return;
                  if (editing == null) {
                    store.addInvestment(name.text.trim(), assetClass, av, rv);
                  } else {
...
Future<void> showAccountForm(
  ...
  final balance = TextEditingController(
    text: editing == null ? '0' : editing.balance.toStringAsFixed(2),
  );
    ...
    builder: (sheetContext) => Padding(
      ...
      child: StatefulBuilder(
        builder: (_, setLocal) => Column(
          ...
          children: [
            Text(
              editing == null ? 'Nova conta' : 'Editar conta',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900),
            ),
            const SizedBox(height: 13),
            TextField(
            ...
            const SizedBox(height: 9),
            DropdownButtonFormField<String>(
              initialValue: types.contains(type) ? type : 'Outro',
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  final bv = parseNumberInput(balance.text) ?? 0;
                  if (name.text.trim().isEmpty) return;
                  final ok = editing == null
                      ? store.addAccount(name.text.trim(), bv, type: type)
                      : store.updateAccount(
                          editing,
                          name.text.trim(),
                          type,
                  ...
                  if (!ok) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'Não foi possível salvar a conta. O nome pode estar em uso ou reservado pelo histórico.',
                        ),
                      ),
                    );
                    return;
...
Future<void> showCardForm(BuildContext context, {CardItem? editing}) async {
  ...
  final used = TextEditingController(
    text: editing == null ? '0' : editing.used.toStringAsFixed(2),
  );
  final close = TextEditingController(
    text: (editing?.closeDay ?? 25).toString(),
  );
  final due = TextEditingController(text: (editing?.dueDay ?? 5).toString());
  var defaultAccount =
      editing?.defaultAccountName ??
      (store.data.accounts.isEmpty ? '' : store.data.accounts.first.name);
      ...
      child: StatefulBuilder(
        builder: (_, setLocal) => SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text(
                editing == null ? 'Novo cartão' : 'Editar cartão',
                style: const TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.w900,
                ),
              ...
              TextField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Fatura/saldo inicial',
                ),
              ),
              const SizedBox(height: 9),
              Row(
              ...
              if (store.data.accounts.isNotEmpty) ...[
                ...
                DropdownButtonFormField<String>(
                  initialValue:
                      store.data.accounts.any((e) => e.name == defaultAccount)
                      ? defaultAccount
                      : store.data.accounts.first.name,
                  decoration: const InputDecoration(
                    labelText: 'Conta padrão para pagar',
                  ),
                  items: store.data.accounts
                      .map(
                        (e) => DropdownMenuItem(
                      ...
                      .toList(),
                  onChanged: (v) {
                    if (v != null) setLocal(() => defaultAccount = v);
                  },
                ),
              ],
              const SizedBox(height: 13),
              SizedBox(
                ...
                child: FilledButton(
                  onPressed: () {
                    final lv = parseNumberInput(limit.text) ?? 0;
                    final uv = parseNumberInput(used.text) ?? 0;
                    final cv = int.tryParse(close.text) ?? 25;
                    final dv = int.tryParse(due.text) ?? 5;
                    if (name.text.trim().isEmpty || lv <= 0) return;
                    final ok = editing == null
                        ? store.addCard(
                            name.text.trim(),
                            lv,
                    ...
                    if (!ok) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Não foi possível salvar o cartão. O saldo informado não pode ficar abaixo das compras ainda pendentes.',
                          ),
                        ),
                      );
                      return;
    ...
    builder: (sheetContext) => Padding(
      ...
      child: StatefulBuilder(
        builder: (_, setLocal) => Column(
          ...
          children: [
            Text(
              editing == null ? 'Nova categoria' : 'Editar categoria',
              style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900),
            ),
            const SizedBox(height: 13),
            TextField(
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  if (name.text.trim().isEmpty) return;
                  final ok = editing == null
                      ? store.addCategory(name.text.trim(), income)
                      : store.updateCategory(editing, name.text.trim(), income);
                  if (!ok) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'Não foi possível salvar a categoria. Categorias em uso não podem trocar entre receita e despesa.',
                        ),
                      ),
                    );
                    return;
  ...
  await showDialog<void>(
    ...
    builder: (dialogContext) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(
          onPressed: () {
            final value = parseNumberInput(controller.text) ?? 0;
            if (value <= 0) return;
            if (goal) {
