    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          SurfaceCard(
            child: Column(
              children: FinanceStore.defaultExpenseCategories
                  .map(
                    (name) => ListTile(
                      dense: true,
                              ...
                              onSelected: (value) async {
                                if (value == 'edit') {
                                  ...
                                  final ok = await confirmAction(
                                    context,
                                    'Excluir categoria?',
                                    'Os lançamentos antigos continuarão com o nome atual.',
                                  );
                                  if (ok &&
                                      !store.deleteCategory(item.id) &&
                                      context.mounted) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text(
                                          'Esta categoria ainda é usada por orçamento, previsão ou recorrência ativa.',
                                        ),
                                      ),
                                    );
                                  }
...
class NotificationCenterScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final upcoming = pending
        .where(
          (item) => !item.date.isBefore(today) && !item.date.isAfter(threshold),
        )
        .toList();
...
class InsightsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final texts = [
      ...
      store.overduePlannedCount == 0
          ? 'Seu planejamento não possui compromissos atrasados.'
          : '${store.overduePlannedCount} compromisso(s) previsto(s) precisam de atenção.',
    ];
            ...
            child: Column(
              children: [
                SwitchListTile(
                  ...
                  subtitle: const Text(
                    'Preto absoluto com detalhes dourados',
