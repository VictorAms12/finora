...
void main() {
  ...
  test('adiar recorrência não recria a ocorrência na data original', () {
    ...
    final first =
        store.data.planned.where((item) => item.recurrenceId != null).toList()
  ...
  test(
    'transferência futura vira previsão e só movimenta saldo ao realizar',
    () {
  ...
  test('renomear categoria propaga para recorrências e parcelamentos', () {
    ...
    store.data.installmentPlans.add(
      InstallmentPlan(
        id: 'installment-1',
        title: 'Compra',
        category: 'Antiga',
