...
void main() {
  ...
  test('meta reduzida preserva valor já acumulado e aceita excedente', () {
    ...
    expect(
      store.updateGoal(goal, 'Notebook', 3000, 4000, DateTime(2027)),
      isTrue,
    );
  ...
  test(
    'disponível considera compromissos reais e não desconta sugestão de meta',
    () {
      final store = baseStore();
      store.data.goals.add(
        GoalItem(
          id: 'g1',
          name: 'Meta',
          target: 1000,
  ...
  test(
    'déficit de compromissos fica visível sem tornar disponível negativo',
    () {
      final store = baseStore();
  ...
  test('editar datas do cartão não move compras históricas de fatura', () {
    ...
    store.addTransaction(
      TransactionItem(
        ...
        cardId: card.id,
        invoiceMonth: historicalInvoice,
      ),
    );
    expect(
      store.updateCard(card, 'Cartão novo', 5000, 200, 10, 5, 'Principal'),
      isTrue,
    );
  ...
  test(
    'cartão não aceita saldo usado abaixo das compras pendentes rastreadas',
    () {
      final store = baseStore();
      final card = CardItem(
        id: 'c1',
  ...
  test('apagar ocorrência recorrente realizada não faz ela reaparecer', () {
    ...
    final tx = store.data.transactions.singleWhere(
    ...
    final occurrence = store.data.planned
        .where(
          (e) =>
              e.recurrenceId != null &&
              store.sameDay(e.canonicalRecurrenceDate, canonical),
        )
        .toList();
  ...
  test('apagar parcela realizada devolve a parcela ao planejamento', () {
    ...
    final tx = store.data.transactions.singleWhere(
      (e) => e.installmentNumber == 1,
    );
    store.deleteTransaction(tx);
    final restored = store.data.planned.singleWhere(
      (e) => e.installmentNumber == 1,
    );
  ...
  test('falha SQLite não deixa estado antigo sobrescrever o espelho mais novo no restart', () async {
    ...
    final legacy = FinanceData.fromJson(
      Map<String, dynamic>.from(
        jsonDecode(prefs.getString('finora_data_v02')!) as Map,
      ),
    );
    ...
    final sqlite = FinanceData.fromJson(
      Map<String, dynamic>.from(
        jsonDecode((await database.readPrimaryRaw())!) as Map,
      ),
    );
